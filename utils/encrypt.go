...
func VideoEncrypt(laby, source string, framerate int, routines int) {
	outputDir := ""
	sourceDir := ""
	...
	if !strings.Contains(source, "/") || !strings.Contains(source, "\\") {
		...
		sourceDir = filepath.Join(os.Getenv("PWD"), filepath.Base(source))
	} else {
...
func Encrypt(laby, source, output string) {
	labyArr := LabyFileToList(laby)
	GenerateRandomImage(labyArr, source, output)
	fmt.Println("Success")
}
